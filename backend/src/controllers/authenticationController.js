...
export async function login(req, res, next) {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { email }, include: { user_details: true } });
...
export async function me(req, res) {
  ...
  const user = await prisma.user.findUnique({ where: { id_user: userId }, include: { user_details: true } });
  if (!user) return res.status(404).json({ error: 'User not found' });
